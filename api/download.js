...
// --- PUSAT PENGENDALI API ---
export default async function handler(req, res) {
    res.setHeader('Access-Control-Allow-Origin', '*');
    
    // Vercel otomatis mengambil "type" dari nama file URL, dan "url" dari input user
    ...
    try {
        if (type === 'tiktok') {
            ...
            res.status(200).json({ status: true, creator: "InuuTyzDev", data: result });
        } 
        else if (type === 'fb') {
            // Logika Facebook
            ...
            res.json({ status: true, creator: "InuuTyzDev", result });
        }
        else if (type === 'mediafire') {
            // Logika Mediafire
            const { data } = await axios.get(url, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/91.0' }});
            const $ = cheerio.load(data);
            const downloadLink = $('#downloadButton').attr('href');
            if (!downloadLink) throw new Error('Gagal menemukan link download.');
            res.json({ status: true, creator: "InuuTyzDev", result: { dl: downloadLink }});
        }
        else {
            res.status(400).json({ status: false, message: "Endpoint tidak valid (gunakan tiktok, fb, ig, atau mediafire)" });
        }
    } catch (e) {
