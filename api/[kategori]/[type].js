...
// ==========================================
    ...
    try {
        // 1. Mapping kata kunci pencarian agar hasilnya akurat
        const queryMap = {
            jkt48: "JKT48 member",
        ...
        // 2. Tembak Bing Images dengan filter khusus situs Pinterest
        const searchUrl = `https://www.bing.com/images/search?q=site:pinterest.com+${encodeURIComponent(targetSearch)}`;
        ...
        // 3. Regex untuk memotong & mengambil link gambar asli dari CDN Pinterest (i.pinimg.com)
        const regex = /https:\/\/i\.pinimg\.com\/[a-zA-Z0-9_\-\/]+\.(jpg|jpeg|png)/g;
        const matches = response.data.match(regex);

        // 4. Jika pencarian spesifik Pinterest gagal, kita matikan filternya (Ambil gambar umum dari Bing agar API tidak 404)
        if (!matches || matches.length === 0) {
            const fallbackUrl = `https://www.bing.com/images/search?q=${encodeURIComponent(targetSearch)}`;
            const fallbackResponse = await axios.get(fallbackUrl, {
                headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' }
            });
            
            // Ambil link gambar umum apa saja yang berformat jpg/png dari hasil Bing
            const generalRegex = /https?:\/\/[^\s"'<>]+?\.(jpg|jpeg|png)/g;
            const fallbackMatches = fallbackResponse.data.match(generalRegex);
            
            if (!fallbackMatches || fallbackMatches.length === 0) {
                return res.status(404).json({ status: false, message: `Foto untuk '${type}' tidak ditemukan!` });
            }

            const uniqueFallback = [...new Set(fallbackMatches)];
            const getRandomFallback = uniqueFallback[Math.floor(Math.random() * uniqueFallback.length)];
            
            return res.status(200).json({
                status: true,
                creator: "InuuTyzDev",
                result: getRandomFallback
            });
        }

        // 5. Jika sukses mendapatkan link Pinterest, bersihkan duplikat dan acak hasilnya
        const uniqueImages = [...new Set(matches)];
        const getRandom = (array) => array[Math.floor(Math.random() * array.length)];
        
        // Saring dan ubah resolusi gambar kecil (236x) menjadi resolusi tajam (736x)
        const finalImage = getRandom(uniqueImages).replace('/236x/', '/736x/');
        ...
        return res.status(500).json({ 
            status: false, 
            message: "Terjadi gangguan saat mengambil data foto!" 
        });
    }
}
