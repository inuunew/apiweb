...
// ==========================================
    ...
    try {
        // --- 1. KHUSUS QUOTES (SCRAPING OTAKOTAKU) ---
        if (type === 'quotes-anime') {
            ...
            $('.quote-item').each((i, el) => {
                hasil.push({
                    quote: $(el).find('.quote-content').text().trim(),
                    karakter: $(el).find('.quote-author').text().trim(),
                    anime: $(el).find('.quote-anime').text().trim()
                });
            });
            if (hasil.length === 0) return res.status(404).json({ status: false, message: "Quotes tidak ditemukan." });
            return res.status(200).json({ status: true, creator, result: getRandom(hasil) });
        }

        // --- 2. LOGIKA AMBIL DATA DARI ANIME.JSON ---
        const dbPath = path.join(__dirname, './database/anime.json');
        if (!fs.existsSync(dbPath)) return res.status(404).json({ status: false, message: "Database anime.json tidak ditemukan!" });
        
        const database = JSON.parse(fs.readFileSync(dbPath, 'utf-8'));
        let imageUrl = null;

        if (type === 'random-anime') {
            // PILIH KATEGORI ACAK: Ambil semua key (waifu, husbu, rem, dll)
            const allCategories = Object.keys(database); 
            const randomCategory = getRandom(allCategories);
            
            // PILIH GAMBAR ACAK dari kategori yang terpilih
            imageUrl = getRandom(database[randomCategory]);
        } else {
            // AMBIL BERDASARKAN TYPE SPESIFIK (Misal: type=rem)
            if (database[type]) {
                imageUrl = getRandom(database[type]);
            }
        }

        // --- 3. PROSES PENGIRIMAN GAMBAR LANGSUNG ---
        if (imageUrl) {
            ...
            return res.status(404).json({ 
                status: false, 
                message: `Type '${type}' tidak terdaftar di anime.json!` 
            });
        }

    } catch (e) {
        console.error(e);
        return res.status(500).json({ status: false, message: "Gagal memproses permintaan anime." });
    }
}
    ...
    try {
        ...
        if (type === 'melolo-home') {
            ...
            if (result.creator) delete result.creator; // Hapus creator luar
            ...
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });

        } else if (type === 'melolo-detail') {
            if (!url) return res.status(400).json({ status: false, message: "Parameter URL wajib diisi!" });
            response = await axios.get(`${baseUrl}/api/movie/melolo-detail?apikey=${apiKeyCuki}&url=${encodeURIComponent(url)}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });
            ...
            response = await axios.get(`${baseUrl}/api/movie/melolo-download?apikey=${apiKeyCuki}&url=${encodeURIComponent(url)}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });
            ...
            response = await axios.get(`${baseUrl}/api/movie/melolo-category?apikey=${apiKeyCuki}&category_url=${encodeURIComponent(url)}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });
            ...
            const p = page || 1;
            response = await axios.get(`${baseUrl}/api/movie/donghua-search?apikey=${apiKeyCuki}&query=${encodeURIComponent(keyword)}&page=${p}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });
            ...
            if (!url) return res.status(400).json({ status: false, message: "Parameter URL wajib diisi!" });
            response = await axios.get(`${baseUrl}/api/movie/donghua-detail?apikey=${apiKeyCuki}&url=${encodeURIComponent(url)}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });

        } else if (type === 'drakor-search') {
            response = await axios.get(`${baseUrl}/api/movie/drakor-search?apikey=${apiKeyCuki}&query=${encodeURIComponent(keyword)}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });

        } else if (type === 'iq-search') {
            response = await axios.get(`${baseUrl}/api/movie/iq-search?apikey=${apiKeyCuki}&query=${encodeURIComponent(keyword)}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });

        } else if (type === 'iq-detail') {
            response = await axios.get(`${baseUrl}/api/movie/iq-detail?apikey=${apiKeyCuki}&query=${encodeURIComponent(keyword)}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });

        } else if (type === 'dramabos-search') {
            response = await axios.get(`${baseUrl}/api/movie/dramabos-search?apikey=${apiKeyCuki}&query=${encodeURIComponent(keyword)}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });
            ...
            response = await axios.get(`${baseUrl}/api/movie/netflix-trending?apikey=${apiKeyCuki}&language=${lang}`);
            result = response.data.result || response.data;
            if (result.creator) delete result.creator;
            return res.status(200).json({ status: true, creator: "InuuTyzDev", result });
