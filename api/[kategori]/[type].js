...
// ==========================================
    ...
    // Daftar type yang ada di dalam anime.json (Catbox links)
    ...
    try {
        // --- 1. LOGIKA ACAK DARI SEMUA (RANDOM-ANIME) ---
        if (type === 'random-anime') {
            const allKeys = Object.keys(animeData);
            const randomKey = getRandom(allKeys);
            const imageUrl = getRandom(animeData[randomKey]);
            
            const response = await axios.get(imageUrl, { responseType: 'arraybuffer' });
            res.set('Content-Type', response.headers['content-type']);
            return res.send(response.data);

        // --- 2. LOGIKA TYPE SPESIFIK DARI JSON (KIRIM GAMBAR) ---
        } else if (localAnimeTypes.includes(type)) {
            const data = animeData[type];
            if (!data) return res.status(404).json({ status: false, creator, message: `Data untuk '${type}' kosong!` });
            
            const imageUrl = getRandom(data);
            const response = await axios.get(imageUrl, { responseType: 'arraybuffer' });
            res.set('Content-Type', response.headers['content-type']);
            return res.send(response.data);

        // --- 3. LOGIKA QUOTES (SCRAPING) ---
            ...
            if (hasil.length === 0) return res.status(404).json({ status: false, creator, message: "Quotes tidak ditemukan." });
            return res.status(200).json({ status: true, creator, result: getRandom(hasil) });

        } else {
            return res.status(404).json({ status: false, creator, message: `Type anime '${type}' tidak ditemukan!` });
        }

    } catch (e) {
        return res.status(500).json({ status: false, creator, message: "Error saat mengambil data anime: " + e.message });
    }
}
