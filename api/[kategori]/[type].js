...
// ==========================================
    ...
    try {
        // 1. Mapping kata kunci pencarian
        const queryMap = {
            jkt48: "JKT48 member",
        ...
        // 2. Scrape Bing Images (Filter Pinterest)
        ...
        if (matches && matches.length > 0) {
            ...
            // Saring dan ubah resolusi ke tinggi (736x)
            imageUrl = getRandom(uniqueImages).replace('/236x/', '/736x/');
        } else {
            // 4. Fallback jika Pinterest kosong (Cari gambar umum)
            const fallbackUrl = `https://www.bing.com/images/search?q=${encodeURIComponent(targetSearch + " wallpaper hd")}`;
            const fallbackResponse = await axios.get(fallbackUrl, {
                headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' }
            });
            
            // Regex diperbarui: Tolak link yang berasal dari www.bing.com agar tidak mengambil ikon web
            const generalRegex = /https?:\/\/(?!www\.bing\.com)[^\s"'<>]+?\.(jpg|jpeg|png)/g;
            const fallbackMatches = fallbackResponse.data.match(generalRegex);
            
            if (!fallbackMatches || fallbackMatches.length === 0) {
                return res.status(404).json({ status: false, creator, message: `Foto untuk '${type}' tidak ditemukan!` });
            }

            const uniqueFallback = [...new Set(fallbackMatches)];
            imageUrl = getRandom(uniqueFallback);
        }

        // --- 5. LOGIKA MENGIRIM GAMBAR LANGSUNG (DIRECT SEND) ---
        if (imageUrl) {
            ...
            // Gunakan setHeader lalu kirim buffer
            ...
            return res.status(404).json({ 
                status: false, 
                creator, 
                message: "Gagal memproses URL gambar." 
            });
        }
        ...
        return res.status(500).json({ 
            status: false, 
            creator, 
            message: "Error mengambil atau merender gambar: " + e.message 
        });
    }
}
